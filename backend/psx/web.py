    ...
    def toframe(self, data):
        ...
        for row in rows:
            ...
            for key, value in zip(self.headers, cols):
                if key == "TIME":
                    try:
                        value = datetime.strptime(value.strip(), "%b %d, %Y")
                    except ValueError:
                        break  # skip malformed rows
                stocks[key].append(value)
        
        if not stocks or "TIME" not in stocks:
            return None  # return None for empty months
    
        return pd.DataFrame(stocks, columns=self.headers).set_index("TIME")

    def daterange(self, start: date, end: date) -> list:
        # Fetch every calendar month spanning start..end *inclusive*. PSX's
        # /historical endpoint returns whole months, so we must request each
        # month between start and end. The previous `period.days // 30`
        # undercounted: an incremental gap that crosses a month boundary in
        # under 30 days (e.g. 05-30 -> 06-03) yielded 0 months and only the
        # start month was fetched, silently dropping the new month's data.
        ...
        return dates if dates else [datetime(start.year, start.month, 1)]
